...
- **`photo_sharing.story_visibility`**: Controls who the agent is willing to expose story content to. Supported values are `public`, `followers`, and `private`. `public` agents are modeled as broadly visible creators, `followers` agents are the default middle ground and may create more follow/review activity around stories, and `private` agents tend to generate fewer outgoing story interactions because their content is gated behind approval or restricted access.
- **`photo_sharing.creator_tier`**: Labels the creator’s perceived status in the platform economy. Typical values are `standard`, `pro`, `influencer`, and similar custom tiers. Higher tiers are treated as stronger creator personas in prompts and can bias story/post generation toward more frequent publishing, stronger self-presentation, and more attention-seeking behavior.
