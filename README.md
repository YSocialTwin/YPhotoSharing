...
1. **Configure your experiment**:
   Navigate to the `example/` directory. Modify `server_config.json`, `client_config.json`, `agents.json`, and `prompts_ygram.json` to define your population size, LLM endpoints, prompt templates, and simulation length. The `agents.json` file accepts YSimulator-style fields plus photo-sharing extras such as `photo_sharing.cover_image` or `photo_sharing.story_visibility`; the prompt persona is then enriched with the demographic and profile data found there. Ensure your `client_config.json` includes simulation parameters for features like `opinion_dynamics` and `discussion_topics` if you wish to use them.
