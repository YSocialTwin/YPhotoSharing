...
1. **Configure your experiment**:
   Navigate to the `example/` directory. Modify `server_config.json`, `client_config.json`, `users.json`, and `prompts_ygram.json` to define your population size, LLM endpoints, prompt templates, and simulation length. Ensure your `client_config.json` includes simulation parameters for features like `opinion_dynamics` and `discussion_topics` if you wish to use them.
